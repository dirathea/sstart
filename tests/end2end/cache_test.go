...
func TestE2E_Cache_PerProviderOverride(t *testing.T) {
	...
	// Create config with global cache enabled but provider cache disabled
	configContent := `
cache:
  enabled: true
  ttl: 1m
...
`
	...
	// Global cache should be enabled
	if !cfg.IsCacheEnabled() {
		t.Fatal("Expected global cache to be enabled")
	}

	// But provider cache should be disabled
	if cfg.IsCacheEnabledForProvider("test-env") {
		t.Fatal("Expected provider cache to be disabled via override")
	}
	...
	// Second collection - should get fresh value since provider cache is disabled
	secrets2, err := collector.Collect(ctx, nil)
	if err != nil {
		t.Fatalf("Failed to collect: %v", err)
	}
